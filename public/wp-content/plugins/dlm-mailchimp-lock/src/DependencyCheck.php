...
class DependencyCheck {
	...
	private function check_dependencies() {
		...
		if( defined( 'MC4WP_VERSION' ) && version_compare( MC4WP_VERSION, '2.5', '>=' ) ) {
			return true;
		}

		// check for mailchimp for wordpress lite
		if( defined( 'MC4WP_LITE_VERSION' ) && version_compare( MC4WP_LITE_VERSION, '2.2', '>=' ) ) {
			return true;
		}
	...
	public function admin_notice() {
		?>
		<div class="updated">
			<p><?php printf( __( 'Please install <a href="%s">%s</a> in order to use %s.', 'mailchimp-sync' ), 'https://wordpress.org/plugins/mailchimp-for-wp/', 'MailChimp for WordPress', 'MailChimp Lock Extension for Download Monitor' ); ?></p>
		</div>
		<?php
	}
}